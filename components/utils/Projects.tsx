...
function calculateProjectScore(project: ProjectProps, userPreferences: UserPreferences | null) {
  ...
  const ageInDays = (new Date().getTime() - project.createdAt.getTime()) / (1000 * 60 * 60 * 24);
  ...
  if (userPreferences?.carrera && project.autor?.carrera === userPreferences.carrera) {
    score += 25; // 25% del score
  }

  // Coincidencia de universidad
  if (userPreferences?.universidad && project.autor?.universidad === userPreferences.universidad) {
...
export default function ListaProyectos({ userId }: ListaProyectosProps) {
          ...
          >
              ...
              {proyecto.tags && proyecto.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {proyecto.tags.map((tag) => (
                    <span key={tag} className="text-xs bg-beige/10 text-beige px-2 py-1 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex justify-around p-4 border-t border-white/10 bg-[#1e1e1e] mt-auto">
              <button className="flex items-center gap-1 text-gray-400 hover:text-beige">
                <ThumbsUp size={18} />
                <span className="text-sm">{proyecto.likes || 0}</span>
              </button>
              <button className="flex items-center gap-1 text-gray-400 hover:text-beige">
